// ==== КОНФИГУРАЦИЯ ====
// Замени на адрес своего задеплоенного бэкенда (PythonAnywhere / Render / свой сервер)
...
const MOTIVATION_LINES = [
  ...
  "Каждая закрытая задача — гвоздь в крышку твоих оправданий.",
...
let state = {
  ...
  tasks: [], // {id, title, deadline, priority, done, subtasks: [{id, title, done}]}
};
...
function renderHome() {
  ...
  const idx = Math.min(
    ...
    Math.floor((progress / 100) * MOTIVATION_LINES.length)
  );
...
function renderPlan() {
  ...
  state.tasks.forEach((task) => {
    ...
    const deadlineHtml = task.deadline
      ? `<span class="text-[11px] text-zinc-500 font-semibold">📅 ${formatDeadline(task.deadline)}</span>`
      : "";
    ...
    card.innerHTML = `
      <div class="flex items-start gap-3">
        <button class="task-checkbox mt-0.5 w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 transition-colors
          ${task.done ? "bg-emerald-500 border-emerald-500" : "border-zinc-600"}"
          data-task-id="${task.id}" ${hasSubtasks ? "disabled" : ""}>
          ${task.done ? '<span class="text-base-900 text-sm font-bold">✓</span>' : ""}
        </button>
        ...
        <button class="task-delete text-zinc-600 hover:text-red-400 text-lg px-1" data-task-id="${task.id}">✕</button>
      </div>
        ...
        ${(task.subtasks || [])
          ...
          <div class="flex items-center gap-2">
            <button class="subtask-checkbox w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0
              ${st.done ? "bg-emerald-500 border-emerald-500" : "border-zinc-600"}"
              data-task-id="${task.id}" data-subtask-id="${st.id}">
              ${st.done ? '<span class="text-base-900 text-[10px] font-bold">✓</span>' : ""}
            </button>
            <p class="flex-1 text-xs text-zinc-300 ${st.done ? "line-through text-zinc-600" : ""}">${escapeHtml(st.title)}</p>
            <button class="subtask-delete text-zinc-700 hover:text-red-400 text-sm" data-task-id="${task.id}" data-subtask-id="${st.id}">✕</button>
          </div>
        `
          )
          .join("")}
        <div class="flex items-center gap-2 mt-1">
          <input type="text" placeholder="Новый пункт..." class="subtask-input flex-1 bg-base-700/60 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-zinc-500 outline-none focus:border-emerald-500" data-task-id="${task.id}" />
...
function updatePriorityButtons() {
  document.querySelectorAll(".priority-btn").forEach((btn) => {
    ...
    if (btn.dataset.priority === "important") {
      btn.className = `priority-btn flex-1 py-3 rounded-xl border font-bold text-sm ${
        active
      ...
      btn.className = `priority-btn flex-1 py-3 rounded-xl border font-bold text-sm ${
        active
          ? "border-sky-500/60 text-sky-400 bg-sky-500/15"
          : "border-zinc-700 text-zinc-500"
      }`;
    }
  });
